...
void trickEnemy(int Player,Integer Center){

if(distancePointsCenter2(adverse(Player))<5  ){
ChangeCenter(Center);
moves();
}
}
int [] a={4,8};
Integer ChangeCenter(Integer Center){
Center=obst[a[(int)random(2)%2]][a[(int)random(2)%2]];
return Center;
}
}
